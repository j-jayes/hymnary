...
if data_path:
    ...
    if is_filtered and clf_data:
        ...
        excel_link = '\n\n📥 **[Download full dataset as Excel](hymn_tune_data.xlsx)** — includes all tune results, AI classification, and metadata.' if excel_path.exists() else ''
...
if data_path and data_path.exists():
    ...
    if summary_csv_path.exists():
        try:
            ...
            # Check if column exists (it might be boolean or string "Yes"/"No" depending on how pandas loaded it vs how I saved it)
            # I saved it as boolean in dataframe before writing to CSV.
            # wait, in filter_pipeline I did: df["in_hymn_book"] = boolean
            # then I wrote to csv.
            # IN THE EXCEL WRITER, I converted to "Yes"/"No". But the CSV writer used the dataframe directly.
            # So CSV has True/False.
    ...
    for hymn in hymn_data:
        ...
        search_total = hymn.get("total_search_results", "—")
        ...
        for i, t in enumerate(tunes):
            ...
            tune_title = detail.get("title", card.get("title", "—"))
            composer = detail.get("composer", "—")
            meter = detail.get("meter", "—")
            key_sig = detail.get("key", "—")
            slug = t.get("tune_slug", "")
            url = f"https://hymnary.org/tune/{slug}" if slug else "#"
            n_hymnals = detail.get("num_hymnals", card.get("num_hymnals", "—"))

            # MIDI link
            midi_url = detail.get("midi_url", "")
            midi_cell = f'<a href="{midi_url}" title="Download MIDI">🎵</a>' if midi_url else ""

            # Book Match Icon
            book_cell = ""
            if slug in book_matches:
                book_cell = '<span title="Match found in Mission Praise" style="font-size: 1.2em;">📖</span>'
